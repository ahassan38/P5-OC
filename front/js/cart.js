const kanapValue = JSON.parse(localStorage.getItem("cartLs"));
/////////////// déclaration de la fonction du fetch pour acceder aux infos Hors Scope/////////
async function fetchApi() {    
let kanapArray = []; // tableau vide qui va contenir les objets créés en suite
let kanapClass = JSON.parse(localStorage.getItem("cartLs"));
if (kanapClass !== null) {
for (let i = 0; i < kanapClass.length; i++) {
	await fetch("http://localhost:3000/api/products/" + kanapClass[i].idSelectedProduct)
		.then((res) => res.json())
		.then((canap) => {
			const article = {
				...
				color: kanapClass[i].colorSelectedProduct,
				quantity: kanapClass[i].quantity,
				alt: canap.altTxt,
				img: canap.imageUrl,
			};
			kanapArray.push(article); //ajout de l'objet article au tableau 
		})
		.catch(function (err) {
			console.log(err);
		});
...
async function showKanap() {
	...
	const kanapValue = JSON.parse(localStorage.getItem("cartLs"));
	if (kanapValue !== null && kanapValue.length !== 0) {
...
function getKanap() {  
    return JSON.parse(localStorage.getItem("cartLs"));
};
...
async function removeKanap() {
	...
	kanapDelete.forEach((article) => {
		article.addEventListener("click", function (event) {
			...
			const searchDeleteKanap = cart.find(  // on cherche l'élément du Ls concerné 
				(element) => element.idSelectedProduct == idDelete && element.colorSelectedProduct == colorDelete
			);
			cart = cart.filter(  // et on filtre le Ls avec l'élément comme modèle
				(item) => item != searchDeleteKanap
			);
			localStorage.setItem("cartLs", JSON.stringify(cart)); // on met à jour le Ls
	...
	if (getKanap() !== null && getKanap().length === 0) {
...
showKanap();         ////// affichage du DOM ( avec rappel du fetchApi //////
removeKanap();		  ////// suppression dynamique des articles du panier et 
